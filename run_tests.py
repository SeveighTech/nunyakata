...
class TestRunner:
    ...
    def run_command(self, command: List[str], description: str, check_return_code: bool = True) -> bool:
    ...
    def check_dependencies(self) -> bool:
        ...
        for pip_cmd in ["pip3", "pip"]:
            try:
                ...
                print(f"{Colors.OKGREEN}✅ Pip package manager found ({pip_cmd}){Colors.ENDC}")
                pip_found = True
                break
            except (subprocess.CalledProcessError, FileNotFoundError):
                continue
    ...
    def run_tests_with_coverage(self) -> bool:
        ...
        return self.run_command(
            ["python3", "-m", "pytest", "tests/", "-v", 
             "--cov=src/nunyakata", "--cov-report=term-missing", 
    ...
    def run_integration_tests(self) -> bool:
        ...
        result = subprocess.run(
            ["python3", "-m", "pytest", "tests/", "-m", "integration", "--collect-only", "-q"],
            cwd=self.project_root,
            capture_output=True,
            text=True
        )
        ...
        if result.returncode == 0 and result.stdout.strip():
            ...
            print(f"{Colors.WARNING}⚠️  No integration tests found - skipping{Colors.ENDC}")
            return True
    ...
    def build_docs(self) -> bool:
        ...
        if (self.project_root / "mkdocs.yml").exists():
            ...
            print(f"{Colors.WARNING}⚠️  No mkdocs.yml found - skipping documentation build{Colors.ENDC}")
            return True
...
Examples:
    ...
    parser.add_argument(
        "--fast", 
        action="store_true", 
        help="Skip slow checks (security, docs, build)"
    )
    parser.add_argument(
        "--coverage", 
        action="store_true", 
        help="Run only tests with coverage"
    )
    parser.add_argument(
        "--no-coverage", 
        action="store_true", 
    ...
    try:
        if args.coverage:
            ...
            success = (
                ...
                runner.install_dev_dependencies() and
            ...
            success = (
                runner.check_dependencies() and
                runner.install_dev_dependencies() and
                runner.run_formatting_check() and
